...
**Versão:** 1.0
**Padrão:** Frontend SPA desacoplado (hoje standalone) + Backend Spring Boot com arquitetura hexagonal (Ports and Adapters)
...
### 3.1 Estilo: Hexagonal (Ports and Adapters), camadas globais

*(Revisão: até esta versão, o backend usava package-by-feature — um pacote por bounded context com suas próprias camadas internas. Migrado para hexagonal com camadas globais para seguir o mesmo padrão de organização de um projeto de referência. Trade-off aceito conscientemente: `domain/model`, `domain/ports`, `application/service` e `adapter/*` são pacotes únicos compartilhados por todos os domínios — conforme os módulos 🔲 abaixo forem implementados, esses pacotes crescem juntos em vez de ficarem isolados por bounded context; se isso virar dor real, revisitar esta decisão.)*
...
│   └── ports/
│       ├── in/                   ✅ GetProfileUseCase, UpdateProfileUseCase, GetPreferencesUseCase, UpdatePreferencesUseCase
│       └── out/                  ✅ ProfileRepository, UserPreferencesRepository (interfaces)
├── application/
│   └── service/                  ✅ ProfileService, UserPreferencesService (implementam os UseCases)
├── adapter/
│   ├── in/
│   │   └── rest/                 ✅ ProfileController, HealthController, request/&lt;contexto&gt;/, response/&lt;contexto&gt;/, mapper/, GlobalExceptionHandler
│   └── out/
│       └── persistence/          ✅ *JpaEntity, *JpaRepository (Spring Data), *PersistenceAdapter, mapper/
├── infrastructure/
...
│
│   -- módulos futuros entram nos mesmos domain/application/adapter acima --
...
| **domain/ports/in** | Interface UseCase por operação de negócio + `record` de Command (dados já resolvidos, sem `SecurityContext`/HTTP). | Implementação — só contrato |
| **domain/ports/out** | Interface que a camada de aplicação depende para persistência/integrações externas, implementada pelos adapters. | Detalhe de infraestrutura (JPA, SQL) |
| **application/service** | Implementa os UseCases; orquestra `domain/ports/out`; fronteira transacional (`@Transactional`); lança exceções de domínio. | Ler `SecurityContextHolder`, conhecer DTO/HTTP/JPA |
| **adapter/in/rest** | Fronteira HTTP: resolve o usuário autenticado (`CurrentUserProvider`), monta o Command, valida (`@Valid`), chama o UseCase, mapeia o resultado para DTO de resposta. Documentação OpenAPI. | Lógica de negócio |
| **adapter/out/persistence** | Entidades JPA, `Spring Data JpaRepository`, mapeamento entidade↔domínio, tradução de exceções de infraestrutura (ex.: `DataIntegrityViolationException` em corrida de provisionamento). | Lógica de negócio |
| **infrastructure/config** | Configuração Spring pura (security filter chain, CORS, auditing, OpenAPI). | Lógica de negócio |
...
| **Hexagonal (Ports and Adapters)** com camadas globais | package-by-feature (um pacote por bounded context) | Segue a mesma organização de camadas de um projeto de referência (`docs/ARCHITECTURE.md`); domínio (`domain/model`, `domain/ports`) isolado de framework/infraestrutura (`adapter/*`), cada operação de negócio testável via interface de UseCase sem Spring. Trade-off aceito: pacotes `domain`/`application`/`adapter` são compartilhados por todos os bounded contexts, não isolados por módulo — se isso virar dor conforme `course`/`gamification`/etc. crescerem, revisitar |
